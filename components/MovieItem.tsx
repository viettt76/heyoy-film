...
export function MovieItem({ movieId, name, slug, thumbUrl, type, isFirst, isLast }: MovieItemProps) {
            ...
            try {
                ...
                if (storedMovieDetail[movieId]) {
                    ...
                    const movieDetail = {
                        ...
                        genres: data.movie.category.map((c: any) => c.name),
    ...
    const handleAddFavoriteMovie = () => {
        ...
        if (storedFavoriteMovies) {
            ...
            if (favoriteMovies.find((m: any) => m.movieId === movieId)) {
                toast.error('Phim đã được thêm vào phim yêu thích rồi');
                return;
            }
            favoriteMovies.push({ movieId, name, slug, thumbUrl, type });
            localStorage.setItem('favoriteMovies', JSON.stringify(favoriteMovies));
        } else {
            localStorage.setItem('favoriteMovies', JSON.stringify([{ movieId, name, slug, thumbUrl, type }]));
        }
                        ...
                        <div className="absolute left-0 right-0 bottom-2 px-3 bg-gradient-to-t from-[#141414] via-[#141414f8] to-[#14141400] shadow-[3px_18px_14px_#0009]">
                            ...
                            <div className="flex justify-between items-center w-full mt-1">
                                <div className="flex gap-x-2">
                                    <Link
                                        href={`/${slug}${type === MovieType.MOVIE ? '' : '/1'}`}
