        ...
        (async () => {
            try {
                ...
                if (!storedCountryList) {
                    ...
                    const countryListData = res.data.data.items.map((c: any) => ({
                        id: c._id,
                        name: c.name,
                        slug: c.slug,
                    }));
        ...
        const fetchMovites = async () => {
            try {
                ...
                if (storedNewlyUpdatedMovies) {
                    ...
                    const newlyUpdatedData = newlyUpdatedRes.data.items.map((m: any) => ({
                        movieId: m._id,
                        name: m.name,
                        slug: m.slug,
                ...
                if (storedGenreList.length <= 0) {
                    ...
                    storedGenreList = genreListRes.data.data.items.map((g: any) => ({
                        name: g.name,
                        slug: g.slug,
                    }));
                ...
                if (storedMoviesByGenres) {
                    ...
                    const moviesByGenresRes = await Promise.all(
                        storedGenreList.slice(0, 3).map(async (g: any) => {
                    ...
                    const moviesByGenresData = moviesByGenresRes.map((l) => {
                        return {
                            ...
                            movies: l.data.data.items.map((item: any) => ({
                                movieId: item._id,
                                name: item.name,
                                slug: item.slug,
